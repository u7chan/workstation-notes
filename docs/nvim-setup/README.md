...
Neovim 設定本体は workstation-config 側で一元管理している。

- 設定配布: `chezmoi`（workstation-config 内）
- プラグイン管理: `lazy.nvim`
- LSP 管理: `mason.nvim`
...
dotfiles の `~/.bashrc` には同じ PATH 設定が含まれる。`chezmoi apply` 後にターミナルを開き直すと、以降のシェルでも有効になる。
...
workstation-config の `chezmoi` で dotfiles を反映する。
